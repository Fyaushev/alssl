...
class LMVMedResnetClassifier(nn.Module):
    def __init__(self, root, num_classes=10, blocks_to_retrain=1):
        super(LMVMedResnetClassifier, self).__init__()
        self.num_classes = num_classes
        # RegNetX-400MF pretrained on ImageNet-1k
        self.backbone = torchvision.models.resnet50(pretrained=True)
        pretrained_weight = torch.load(Path(root) / 'lvmmed_resnet.torch', weights_only=True)
        self.backbone.load_state_dict(pretrained_weight, strict=False)
        self.backbone.fc = nn.Identity()

        self.classifier = nn.Linear(in_features=2048, out_features=num_classes, bias=True) # same as in the orig model
        ...
        if blocks_to_retrain == 1:
            block = self.backbone.layer4
            
            print(f'Unfreeze layer 4')
            for pname, params in block.named_parameters():
                if 'bn' not in pname:
                    params.requires_grad = True
        else:
            print(f'Layer 4 is freezed')
...
class LightningLMVMedResnetClassifier(L.LightningModule):
    def __init__(
        self,
        root:str,
        learning_rate=0.001,
        ...
        super().__init__()
        self.model = LMVMedResnetClassifier(num_classes=num_classes, blocks_to_retrain=blocks_to_retrain, root=root)
