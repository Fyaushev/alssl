...
class DinoSegmentation(nn.Module):
    def __init__(self, num_classes=10, blocks_to_retrain=0, use_n_blocks=1):
        super(DinoSegmentation, self).__init__()
        self.use_n_blocks = use_n_blocks
        self.backbone = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14_lc").backbone
        self.decoder = BirefNetDecoder(in_channels=768, num_classes=num_classes)
    ...
    def forward(self, x):
        ...
        features = torch.cat([emb for emb, _ in intermediate_output], dim=-1)
...
class LightningDinoSegmentation(L.LightningModule):
    def __init__(
        ...
        *args
        ...
        super().__init__()
        self.model = DinoSegmentation(num_classes=num_classes, blocks_to_retrain=blocks_to_retrain)
