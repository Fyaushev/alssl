...
class TypiStableStrategy(BaseStrategy):
    MIN_CLUSTER_SIZE = 5
    MAX_NUM_CLUSTERS = 500
    K_NN = 50

    def __init__(self, num_classes: int, cluster_curr: bool = False, add_nn: bool = False):
    ...
    def select_ids(self, model: nn.Module, dataset: ALDataModule, budget: int, almodel: BaseALModel, *args) -> list:
        ...
        features = features_e1 if self.cluster_curr else features_e0
        ...
        clusters_df = clusters_df.sort_values(['clust_consist_score', 'existing_count', 'neg_cluster_size',])
