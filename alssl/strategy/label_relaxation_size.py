...
class LabelRelaxSizeStrategy(BaseStrategy):
    MIN_CLUSTER_SIZE = 5
    ...
    def __init__(self, num_classes: int, size_ratio=1.5, cluster_curr: bool = False, mode: str = 'both', cluster_mode: str = 'both', inverse_score: bool = False, inverse_cluster_score: bool = False, source='e0', load_from_prev_iter=False):
    ...
    def select_ids(self, model: nn.Module, dataset: ALDataModule, budget: int, almodel: BaseALModel, *args) -> list:
        ...
        # priority for bigger clusters
        clusters_df = clusters_df.sort_values(['cluster_size'], ascending=False)
        clusters_df = clusters_df.iloc[:int(self.num_classes * self.size_ratio)]
