...
def get_num_classes():
    len(INFO["breastmnist"]["label"])
    return 2


class BreastMNISTDataset(BreastMNIST):
...
def get_dataset(
    subset="train", data_path=Path("/shared/projects/active_learning/breastmnist")
):
    assert subset in ["train", "test"]
    transform = transform_train if subset == "train" else transform_test
    ds = BreastMNISTDataset(root=data_path, split=subset, download=True)
    return ds, transform
