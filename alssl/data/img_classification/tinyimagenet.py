...
def sort_val_images(data_path: Path):
    ...
    with open(data_path / "val" / "val_annotations.txt") as f:
        for line in f:
            ...
            (data_path / "val_sorted" / cls / "images").mkdir(
                exist_ok=True, parents=True
            )
            shutil.copyfile(
                data_path / "val" / "images" / img_name,
                data_path / "val_sorted" / cls / "images" / img_name,
            )
...
def get_dataset(
    subset="train", data_path=Path("/shared/projects/active_learning/tinyimagenet")
):
