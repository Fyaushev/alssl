...
def get_num_classes():
    len(INFO["pneumoniamnist"]["label"])
    return 2


class PneumoniaMNISTDataset(PneumoniaMNIST):
...
def get_dataset(
    subset="train", data_path=Path("/shared/projects/active_learning/pneumoniamnist")
):
    assert subset in ["train", "test"]
    transform = transform_train if subset == "train" else transform_test
    ds = PneumoniaMNISTDataset(root=data_path, split=subset, download=True)
    return ds, transform
