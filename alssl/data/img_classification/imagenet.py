...
def get_dataset(subset="train", data_path=Path("/shared/projects/active_learning/imagenet")):
    assert subset in ["train", "test"]
    transform = transform_train if subset == "train" else transform_test
    # download_tinyImg200(data_path)
    # sort_val_images(data_path)
    # if subset == "train":
    #     data_path = data_path / "ILSVRC2012_img_train.tar"
    # else:
    #     data_path = data_path / "ILSVRC2012_img_val"
    subset = subset if subset == "train" else "val"
    return torchvision.datasets.ImageNet(
        root=data_path, split=subset, transform=transform
    )