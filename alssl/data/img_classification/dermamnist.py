...
def get_num_classes():
    len(INFO["dermamnist"]["label"])
    return 7


class DermaMNISTDataset(DermaMNIST):
...
def get_dataset(
    subset="train", data_path=Path("/shared/projects/active_learning/dermamnist")
):
    assert subset in ["train", "test"]
    transform = transform_train if subset == "train" else transform_test
    ds = DermaMNISTDataset(root=data_path, split=subset, download=True)
    return ds, transform
