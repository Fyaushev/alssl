...
def get_dataset(
    subset="train", data_path=Path("/shared/projects/active_learning/octmnist")
):
    assert subset in ["train", "test"]
    transform = transform_train if subset == "train" else transform_test
    ds = OCTMNISTDataset(root=data_path, split=subset, download=True)
    return ds, transform
